# -*- coding: utf-8 -*-
"""
Created on Sat Jun 18 18:31:48 2022
...
for i in range(len(sentences)):
    ...
    stopwords_removed = [stemmer.stem(word) for word in words if word not in set(stopwords.words('english'))]
    sentences[i] = ' '.join(stopwords_removed)     
    
    
### Obtain the word tokens. For each tokens we will select those words that are not a stopword and we will stem them. Here set is taken so that we get the unique value.